...
class PermissionsController extends AppController {
	...
	function trash() {
		...
		$data->read('http://localhost/eventgrouper/xlsparser/cpwevents.xls');
		
		for ($i = 2; $i <= $data->sheets[0]['numRows']; $i++) {
			$row = $data->sheets[0]['cells'][$i];
			$timeStart = date("Y-m-d H:i:s", strtotime($row[1]." ".$row[2]));
			$timeEnd = date("Y-m-d H:i:s", strtotime($row[1]." ".$row[3]));
